(() => {
  ...
  const REPLY_API_PATH = '/api/v3/settings/reply_shop_rating/';
...
Nossa equipe está à disposição para verificar o problema e orientar a melhor solução. Por favor, nos chame pelo chat do pedido.`
  ...
  function installShopeeReplyApiWatcher() {
    // O watcher da API de resposta da Shopee é carregado pelo manifest.json
    // em `world: MAIN`, antes do content script principal. Isso permite
    // capturar o fetch/XHR real da página, sem injetar código inline.
    window.__ldbReplyApiWatcherExpected = true;
  }

  function waitForShopeeReplyApiResult(startedAt, timeoutMs = 12000) {
    return new Promise(resolve => {
      let done = false;

      const cleanup = () => {
        done = true;
        window.removeEventListener('ldb:reply-api-result', handler);
        clearTimeout(timer);
      };
      ...
      const handler = event => {
        ...
        let payload = null;
        try { payload = JSON.parse(event.detail); } catch (error) { return; }

        if (!payload || payload.at < startedAt - 1000) return;

        cleanup();

        const body = payload.body || {};
        const message = String(body.message || body.user_message || '');
        const success = payload.ok === true && Number(body.code) === 0 && /success/i.test(message);

        resolve({
          success,
          status: payload.status,
          code: body.code,
          message: body.user_message || body.message || '',
          body
        });
      };
      ...
      window.addEventListener('ldb:reply-api-result', handler);
    });
  }

  async function waitForReplyModalToDisappear(timeoutMs = 3000) {
    const start = Date.now();

    while (Date.now() - start < timeoutMs) {
      if (!isReplyTextareaVisible()) return true;
      await sleep(200);
    }
  ...
  function isReplyTextareaVisible() {
    const textareas = Array.from(document.querySelectorAll('textarea[name="comment"], textarea.eds-react-input__textarea'));
  ...
  async function processOne(button, settings) {
    ...
    const apiResultPromise = waitForShopeeReplyApiResult(sendStartedAt, 12000);
    ...
    if (apiResult && apiResult.success) {
      ...
      updateProgress();
      setShopeeReplyControlsHidden(false);
      log(`Resposta enviada no pedido ${orderId}. Confirmação da Shopee: success.`);
    ...
    if (apiResult && !apiResult.success) {
      ...
      updateProgress();
      setShopeeReplyControlsHidden(false);
      log(`Falha ao enviar resposta do pedido ${orderId}. Shopee retornou: ${apiResult.message || 'erro desconhecido'}.`);
      return 'error';
    }

    const closedQuickly = await waitForReplyModalToDisappear(3000);
    ...
    if (closedQuickly) {
      ...
      updateProgress();
      setShopeeReplyControlsHidden(false);
      log(`Resposta enviada no pedido ${orderId}. Confirmação visual detectada.`);
      await delaySeconds(settings.afterSubmitMin, settings.afterSubmitMax);
      return 'sent';
    }
    ...
    updateProgress();
    setShopeeReplyControlsHidden(false);
    log(`Envio acionado no pedido ${orderId}, mas a extensão não conseguiu confirmar resposta da API nem fechamento visual.`);
    return 'error';
  }
